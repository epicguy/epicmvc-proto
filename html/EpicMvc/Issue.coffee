...
class Issue
	...
	add: (type,msgs) ->
		...
		_log2 f, @issue_list[@issue_list.length- 1]
	...
	asTable: (map) ->
		...
		for issue in @issue_list
			final.push issue: @map map, issue.t_view, issue.t_action, issue.token, issue.more
		final
	map: (map,t_view,t_action,token,more) ->
		...
		if t_view of map
			if t_action of map[t_view]
				map_list.push map[t_view][t_action]; _log2 'm', t_view, t_action
			if 'default' of map[t_view]
				map_list.push map[t_view].default; _log2 'm', t_view, 'default'
		if 'default' of map
			if t_action of map.default
				map_list.push map.default[t_action]; _log2 'm', 'default', t_action
			if 'default' of map.default
				map_list.push map.default.default; _log2 'm', 'default', 'default'

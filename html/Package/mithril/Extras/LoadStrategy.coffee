...
class LoadStrategy
	...
	missing: (type,nm) -> -> m 'span', {style: {color: 'red'}}, "No such #{type} called #{nm}."
	clearCache: () -> @cache= {}
	...
	get: (type,nm) ->
		...
		for pkg in @reverse_packages
			if false and p= @preLoaded pkg, type, nm # TODO DON'T LOOK AT PRECOMPILED STUFF, SINCE IT IS NOT YET RECOMPILED
				...
				results= window.EpicMvc.Extras.ParseFile$mithril full_nm, results if results isnt false
	...
	getFile: (pkg,nm) ->
		...
		path= "EpicPkg/#{pkg}/view/" if pkg in ['Base', 'BaseDevl', 'bootstrap', 'react', 'mithril']
		window.$.ajax
			url: path+ nm
			async:false
...
window.EpicMvc.Extras.LoadStrategy$mithril= LoadStrategy
